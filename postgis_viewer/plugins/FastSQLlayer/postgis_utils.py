...
class DbError(Exception):
	def __init__(self, error):
		...
		if hasattr(error, "cursor") and hasattr(error.cursor, "query"):
			self.query = unicode(str(error.cursor.query), 'utf-8')
		else:
			self.query = None
...
class GeoDB:
	...
	def get_srid_from_geom(self, geomFieldName, query):
		if not self.has_postgis:
			return "Unknown"
		
		try:
			c = self.con.cursor()
			self._exec_sql(c, "select getsrid(foo.%s) from (%s) as foo limit 1" % (geomFieldName, query))
			sr = c.fetchone()
			if sr is None:
				return "-1"
			srtext = sr[0]
